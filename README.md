...
当前版本会随发布提交内置实际存在的可直接使用和修改的规则；规则块与受托管脚本的数量以该提交为准。规则保存在可编辑的 `@mapping` 注释区域中；便携版和源码版都能查看、备份和修改这些内容。项目集成 Interception x64 API、DLL、驱动一键安装、设备感知录制和集中式设备规则运行时；启用的规则只在小助手运行期间生效。
...
<p align="center">
  <img src="docs/images/key-mouse-remapper-assistant-overview.png" alt="键鼠重映射小助手深色主界面" width="100%">
</p>
<p align="center">
  <img src="docs/images/key-mouse-remapper-assistant-overview-light.png" alt="键鼠重映射小助手浅色主界面" width="100%">
</p>
...
- 集成 Interception：可在设置页提权安装或卸载驱动，并在默认创建区勾选“区分来源设备”后录制和生成指定物理键盘／鼠标的规则。
...
- Interception 驱动可由小助手在用户明确确认后请求管理员权限并自动安装或卸载，操作后必须重启 Windows。没有驱动时普通规则和 Raw Input 仍可用，设备区分与底层拦截不可用；即使安装驱动，也无法处理 Windows 安全桌面或 `Ctrl+Alt+Delete` 等安全注意序列。
...
5. 发布提交中的全部内置规则会随完整发行包提供。建议先暂停不需要的规则，再逐条调整为自己的工作流。
...
1. 点击“点击录制来源按键”，按下要被替换的键或组合键。
2. 点击“点击录制目标按键”，按下希望发送的键或组合键；目标可留空以屏蔽来源按键。
3. 填写名称，按需勾选“区分左/右侧修饰键”和“区分来源设备”，然后保存映射。

当目标只录制为一个按键时，新建简易规则会按“替代目标键”处理：例如
`CapsLock -> A` 同时支持 `CapsLock`、`Ctrl+CapsLock`、`Shift+CapsLock`、
`Alt+CapsLock` 和 `Win+CapsLock`，后四种会分别产生对应的
`Ctrl+A`、`Shift+A`、`Alt+A` 和 `Win+A`。来源按下时目标键保持按下，来源
松开时才释放目标键，因此也支持先按住来源、再按其他键组成快捷键，例如
`AppsKey -> Win` 后使用 `AppsKey+E` 打开资源管理器。目标本身是组合键时，
仍按录制的完整按键序列发送。

“区分左/右侧修饰键”同时作用于来源和目标：勾选后保留录制到的左／右侧别；
未勾选时，来源的左右修饰键按同族匹配，目标修饰键则确定性地归一为左侧
`LCtrl`、`LShift`、`LAlt` 或 `LWin`，界面摘要显示为不带侧别的名称。
...
从“设置 → 设备过滤驱动”查看驱动状态和操作入口。首次使用可点击“安装驱动”，驱动就绪后可点击“卸载驱动”；确认 UAC 后由小助手运行发行包内的官方安装器，操作成功后重启 Windows。项目配置编号使用 1..10 表示键盘、11..20 表示鼠标。

创建普通映射时，先勾选“区分来源设备”，再点击“点击录制来源按键”并在目标物理设备上按键；录制详情会显示 Interception 设备编号和硬件 ID。保存后，规则的 `from.device` 会记录 `backend`、`number`、`type` 和诊断用 `hardware_id`，运行时按 `number` 过滤。所有此类规则共用一个集中式 context，未匹配输入会原样回送，不会为每条规则启动互相竞争的 consumer。目标为单键时仍使用真正的 `key_down`／`key_up` 配对，因此来源键可以代替目标键参与组合键。目标按键可以留空；保存后会生成 `block=true` 规则，来源按键的按下和释放都会被屏蔽且不会产生输出。

内置“屏蔽外接键盘 Win 键”规则演示了 `from.device` 与 `block=true` 的组合。发行包内 DLL 位于 `third_party/interception/library/x64/interception.dll`，安装器位于 `third_party/interception/command-line-installer/install-interception.exe`；使用前应阅读 `third_party/interception/licenses/` 下的许可证。
...
- 包含发布提交中的全部内置规则，但不包含构建电脑后来产生的个人规则版本。
...
| 路径 | 职责 |
| --- | --- |
| `键鼠重映射小助手.ahk` | 发布入口与提交中的全部 `@mapping` 规则 |
| `app/` | 应用编排、主窗口、编辑器、设置、帮助和事件界面 |
| `src/Core/` | 规则解析、编译、运行、AI、历史、规则包和应用更新 |
| `src/Input/` | Raw Input 观察、输入录制与录制保护 |
| `third_party/interception/` | 官方 Interception x64/x86 库、安装器、样例和许可证 |
...
带 from.device 的规则块 -> InterceptionMappingRuntime -> 单一 interception context -> 指定设备
...
构建固定生成完整便携 ZIP、完整源码 ZIP 和可选 `fonts.zip`。两个程序包都不含字体，并包含发布提交中的全部内置规则、README、更新日志和许可证；源码版额外包含测试，便携版额外包含编译 EXE、固定 AutoHotkey v2 x64 运行时及对应源码归档。字体包保持 `assets/fonts` 布局，供用户安装到 Windows。发布脚本拒绝用户状态文件和本机 AI 参数，并为同一输入生成确定性 ZIP。
