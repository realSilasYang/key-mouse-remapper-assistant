...
- 集成 Interception：可在设置页提权安装驱动，并在默认创建区勾选“区分来源设备”后录制和生成指定物理键盘／鼠标的规则。
...
- Interception 驱动可由小助手在用户明确确认后请求管理员权限并自动安装，安装后必须重启 Windows。没有驱动时普通规则和 Raw Input 仍可用，设备区分与底层拦截不可用；即使安装驱动，也无法处理 Windows 安全桌面或 `Ctrl+Alt+Delete` 等安全注意序列。
...
从“设置 → 设备过滤驱动”查看驱动状态和安装入口。首次使用可点击“安装驱动”，确认 UAC 后由小助手运行发行包内的官方安装器；安装成功后重启 Windows。项目配置编号使用 1..10 表示键盘、11..20 表示鼠标。
