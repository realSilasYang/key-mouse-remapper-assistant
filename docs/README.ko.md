...
규칙은 읽고 백업할 수 있는 `@mapping` 주석 영역에 저장됩니다. 장치별 Interception 규칙에는 관리자 승인과 Windows 재시작 후 설치하는 선택적 드라이버를 사용할 수 있으며, 매핑은 도우미가 실행 중일 때만 작동합니다. 공식 패키지는 빌드 컴퓨터의 AI 주소, API 키, 모델, 사용자 프롬프트 또는 개인 설정을 포함하지 않습니다.
...
- 일반 규칙은 선택적 Interception 드라이버 없이도 작동합니다. 보안 데스크톱과 `Ctrl+Alt+Delete`는 드라이버를 설치해도 처리할 수 없습니다.
