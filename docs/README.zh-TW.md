...
規則儲存在可讀、可備份的 `@mapping` 註解區域。指定裝置的 Interception 規則可使用選用驅動程式；安裝需要管理員確認並重新啟動 Windows。映射只會在小助手執行時生效。正式發布包不會包含建置電腦的 AI 位址、API 金鑰、模型、自訂提示詞或其他個人設定。
...
- 一般規則不需安裝選用的 Interception 驅動程式。即使已安裝，安全桌面與 `Ctrl+Alt+Delete` 仍不受支援。
