...
キーボード・マウス再マッピングアシスタントは、Windows 10／11 x64 向けの AutoHotkey v2 デスクトップツールです。入力記録、ルール管理、コード編集、AI による生成と最適化、ローカル検証、実行状態を一つの画面にまとめています。各リリースには、そのリリースコミットに存在する編集可能な組み込みルールが含まれます。ルールブロックと管理スクリプトの数はリリースごとに異なる場合があります。

ルールは読み取り・バックアップ可能な `@mapping` コメント領域に保存されます。デバイスを指定する Interception ルールでは、管理者の承認と Windows の再起動を伴う任意のドライバーを利用できます。マッピングはアシスタントの実行中だけ有効です。公式パッケージに、ビルド環境の AI アドレス、API キー、モデル、独自プロンプト、個人設定が入ることはありません。
...
- 通常のルールは任意の Interception ドライバーなしでも動作します。安全なデスクトップと `Ctrl+Alt+Delete` は、ドライバーを導入しても対象外です。
...
どちらのプログラム ZIP にもフォントは含まれません。任意の `fonts.zip` は Noto の代替フォントを提供します。必要なフォントを Windows にインストールすると、アシスタントが既存の優先順位で選択します。プログラムは Windows にインストール済みのフォントだけを列挙し、ZIP やアプリケーションフォルダーから私的に読み込みません。フォントは実行に必須ではありません。
...
ルールは実行フォルダーの `键鼠重映射小助手.ahk`、AI・表示・起動設定は `%APPDATA%\KeyMouseRemapperAssistant` にあります。両方をバックアップしてください。公式パッケージはリリースコミットに存在するルールを含みますが、`settings.ini`、`runtime.ini`、`rule-appearance.json`、`window-layout.ini`、ローカル AI パラメーターを除外します。テレメトリや自動アップロードはありません。
...
ビルドは完全ポータブル ZIP、完全ソース ZIP、任意の `fonts.zip` を生成します。二つのプログラムパッケージにはフォントを含めず、フォント ZIP は Windows へのインストール用です。個人状態やローカル AI パラメーターを拒否して決定論的 ZIP を作成します。規約は[変更履歴テンプレート](changelog-template.md)と[リリース手順](release-process.md)を参照してください。
