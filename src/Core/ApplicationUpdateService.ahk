...
class ApplicationUpdateService {
    ...
    static CheckTimeoutMs := 140000
    static MaximumResultBytes := 64 * 1024

    __New(options) {
        if !IsObject(options)
            throw TypeError("应用更新参数无效")
        this.Repository := String(options.Repository)
        if !ApplicationUpdateService.IsCanonicalRepository(this.Repository)
            throw ValueError("应用更新仓库标识无效")
        ...
        this.EditableSourcePath := options.HasOwnProp("EditableSourcePath")
            ? options.EditableSourcePath : ""
    ...
    BeginCheck(interactive := false, ownerGui := "") {
        ...
        this.WorkDirectory := A_Temp "\KeyMouseRemapperUpdateCheck-"
    ...
    Poll(*) {
        ...
        if workerAlive {
            ...
            if elapsed < ApplicationUpdateService.CheckTimeoutMs
                return
        }
        if workerAlive {
            if this.WorkerHandle
    ...
    ReadResult(resultPath) {
        if !FileExist(resultPath) || DirExist(resultPath)
                || FileGetSize(resultPath)
                    > ApplicationUpdateService.MaximumResultBytes
    ...
    BeginInstall(result) {
        ...
        if !FileExist(this.HelperLocalizationPath)
            throw Error("应用更新本地化资源不存在")
        processId := DllCall("kernel32\GetCurrentProcessId", "UInt")
        helperDirectory := A_Temp "\KeyMouseRemapperUpdateApply-"
            . processId "-" this.Now.Call()
        ...
        try {
            ...
            command := this.BuildPowerShellCommand(temporaryHelper, [
                ...
                "-EditableSourcePath", this.EditableSourcePath,
    ...
    ValidateCheckResult(result, requireAvailable := false) {
        ...
        if comparison <= 0
            throw ValueError("更新检查未返回结果")
        if status == "available" {
            this.ValidateReleaseUrls(result)
    ...
    ValidateReleaseUrls(result) {
        releaseBase := "https://github.com/" this.Repository "/releases"
        downloadBase := releaseBase "/download/" result.Tag "/"
        if result.ReleaseUrl != releaseBase "/tag/" result.Tag
            throw ValueError("更新检查未返回结果")
        expectedUrls := Map(
            "BinaryUrl", downloadBase "key-mouse-remapper-assistant-"
                . result.Version "-windows-x64.zip",
            "SourceUrl", downloadBase "key-mouse-remapper-assistant-"
                . result.Version "-source.zip",
            "ChecksumsUrl", downloadBase "SHA256SUMS.txt")
        for fieldName, expectedUrl in expectedUrls {
            actualUrl := result.%fieldName%
    ...
    static IsCanonicalRepository(repository) {
        parts := StrSplit(String(repository), "/")
        if parts.Length != 2
            return false
        for part in parts {
            if part == "." || part == ".." || !RegExMatch(part,
