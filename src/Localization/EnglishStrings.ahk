...
class EnglishStrings {
    static Create() {
        ...
        catalog.Set("卸载 Interception 驱动", "Uninstall Interception driver")
        catalog.Set("卸载驱动", "Uninstall driver")
        catalog.Set("Interception 驱动当前已就绪。卸载后需要重启 Windows 才能生效。是否继续？", "The Interception driver is ready. You must restart Windows for the uninstall to take effect. Continue?")
        catalog.Set("Interception 驱动卸载失败：{1}", "Failed to uninstall the Interception driver: {1}")
        catalog.Set("Interception 驱动卸载程序已成功返回。请重启 Windows 后生效。", "The Interception uninstaller completed successfully. Restart Windows for the change to take effect.")
