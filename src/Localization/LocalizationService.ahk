...
class LocalizationService {
    ...
    static Configure(language := "auto", uiFont?) {
        ...
        ; 设置事务可同时提交语言和内容字体；只切换语言时，明确选择的字体保持不变。
        if IsSet(uiFont)
            this.ConfigureUiFont(uiFont)
        else if this.UiFont != "" && this.RequestedUiFont == "auto"
    ...
    static NormalizeLanguage(language, fallback := "auto") {
    ...
    static IsFontInstalled(fontName) {
        return this.FindInstalledUiFontName(fontName) != ""
    }
