...
class VietnameseStrings {
    static Create() {
        ...
        catalog.Set("卸载 Interception 驱动", "Gỡ cài đặt trình điều khiển Interception")
        catalog.Set("卸载驱动", "Gỡ cài đặt trình điều khiển")
        catalog.Set("Interception 驱动当前已就绪。卸载后需要重启 Windows 才能生效。是否继续？", "Trình điều khiển Interception đã sẵn sàng. Bạn phải khởi động lại Windows để áp dụng việc gỡ cài đặt. Tiếp tục?")
        catalog.Set("Interception 驱动卸载失败：{1}", "Không thể gỡ cài đặt trình điều khiển Interception: {1}")
        catalog.Set("Interception 驱动卸载程序已成功返回。请重启 Windows 后生效。", "Trình gỡ cài đặt Interception đã hoàn tất thành công. Hãy khởi động lại Windows để áp dụng thay đổi.")
