...
class SpanishStrings {
    static Create() {
        ...
        catalog.Set("卸载 Interception 驱动", "Desinstalar controlador Interception")
        catalog.Set("卸载驱动", "Desinstalar controlador")
        catalog.Set("Interception 驱动当前已就绪。卸载后需要重启 Windows 才能生效。是否继续？", "El controlador de Interception está listo. Debe reiniciar Windows para aplicar la desinstalación. ¿Continuar?")
        catalog.Set("Interception 驱动卸载失败：{1}", "No se pudo desinstalar el controlador de Interception: {1}")
        catalog.Set("Interception 驱动卸载程序已成功返回。请重启 Windows 后生效。", "El desinstalador de Interception finalizó correctamente. Reinicie Windows para aplicar el cambio.")
