...
class SvgRenderLibrary {
    ...
    RenderFile(filePath, dpi, renderSize) {
        ...
        try {
            ...
            try svgData := FileRead(filePath, "RAW")
                ...
                return 0
            if !this.IsValidSourceBuffer(svgData, sourceSize)
                return 0
    ...
    IsValidSourceBuffer(svgData, expectedSize) {
        if Type(svgData) != "Buffer"
            return false
        actualSize := svgData.Size
        return actualSize > 0
            && actualSize <= SvgRenderLibrary.MaximumInputBytes
