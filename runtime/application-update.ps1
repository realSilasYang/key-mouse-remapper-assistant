...
# AppData 配置不进入更新清单；入口源码中的重映射区域会在替换后原样迁移。
...
param(
    ...
    [string]$Mode,
    [string]$Repository = 'realSilasYang/key-mouse-remapper-assistant',
...
$script:MaximumUpdateDownloadBytes = 256MB
$script:MaximumExpandedArchiveBytes = 256MB
...
function Invoke-GitHubApi {
    ...
    $headers = @{
        ...
        'User-Agent' = 'key-mouse-remapper-assistant-updater'
...
function Invoke-HttpDownloadAttempt {
    param(
        ...
        [long]$MaximumBytes = $script:MaximumUpdateDownloadBytes
    )
    ...
    if ($existingLength -gt $MaximumBytes) {
        throw [System.IO.InvalidDataException]::new(
            "Partial update download exceeds the size limit: $existingLength")
    }
    ...
    [void]$request.Headers.UserAgent.ParseAdd('key-mouse-remapper-assistant-updater')
    ...
    try {
        ...
        if ($statusCode -eq 206) {
            ...
            if ($null -eq $contentRange -or -not $contentRange.From.HasValue -or
                $contentRange.From.Value -ne $existingLength) {
                $rangeError = [System.Net.Http.HttpRequestException]::new(
                    "Invalid HTTP range response while downloading $Uri")
        ...
        $totalLength = if ($append) { [long]$existingLength } else { 0L }
        ...
        while ($true) {
            ...
            if ($totalLength -gt $MaximumBytes) {
                throw [System.IO.InvalidDataException]::new(
                    "Update download exceeds the size limit: $MaximumBytes")
            }
        ...
        try {
            if ($normalizedExpectedHash -and
                ...
                Invoke-HttpDownloadAttempt -Uri $Uri -PartialPath $partialPath `
                    -TimeoutSeconds $TimeoutSeconds -MaximumBytes $MaximumBytes
            }
...
function Invoke-Check {
    if (-not (Test-CanonicalRepository $Repository)) {
        throw (Get-UpdateText '更新仓库标识无效：{0}' `
            'Update repository identifier is invalid: {0}' @($Repository))
    }
    ...
    $resultVersion = if ($status -eq 'available') {
        $latestVersion
    } else {
        $CurrentVersion
    }
    $resultTag = "v$resultVersion"
    $binaryName = "key-mouse-remapper-assistant-$latestVersion-windows-x64.zip"
    $sourceName = "key-mouse-remapper-assistant-$latestVersion-source.zip"
    ...
    if ($status -eq 'available') {
        $values.ReleaseUrl = [string]$release.html_url
        $values.BinaryUrl = Get-ReleaseAssetUrl $binaryAsset
        $values.SourceUrl = Get-ReleaseAssetUrl $sourceAsset
        $values.BinarySha256 = Get-ReleaseAssetSha256 $binaryAsset
        $values.SourceSha256 = Get-ReleaseAssetSha256 $sourceAsset
        $values.ChecksumsUrl = Get-ReleaseAssetUrl $checksumsAsset
    }
    ...
    if ($status -eq 'available') {
        Assert-TrustedReleaseUrl $values.ReleaseUrl `
            "https://github.com/$Repository/releases/tag/$resultTag"
        Assert-TrustedReleaseUrl $values.BinaryUrl `
            "https://github.com/$Repository/releases/download/$resultTag/$binaryName" `
            -Optional
        Assert-TrustedReleaseUrl $values.SourceUrl `
            "https://github.com/$Repository/releases/download/$resultTag/$sourceName" `
            -Optional
        Assert-TrustedReleaseUrl $values.ChecksumsUrl `
            "https://github.com/$Repository/releases/download/$resultTag/SHA256SUMS.txt" `
            -Optional
    }
...
function Test-CanonicalRepository {
    param([string]$Value)
    ...
    foreach ($part in $parts) {
        if ($part -in @('.', '..') -or
            $part -cnotmatch '^[A-Za-z0-9][A-Za-z0-9_.-]{0,99}$') {
            return $false
        }
    }
    return $true
...
function Assert-TrustedReleaseUrl {
    param(
    ...
    if (-not $Actual -and $Optional) {
        return
    }
    if ($Actual -cne $Expected) {
        throw (Get-UpdateText '发行附件地址无效：{0}' `
            'Release asset URL is invalid: {0}' @($Actual))
    }
}

$script:RemappingRegionStart = '; === 重映射代码区域开始 ==='
$script:RemappingRegionEnd = '; === 重映射代码区域结束 ==='

function Get-RemappingRegionParts {
    param([Parameter(Mandatory = $true)][string]$Text)

    if ($Text.Length -gt 12MB) {
        throw (Get-UpdateText '重映射代码区域超过大小上限。' `
            'The remapping source exceeds the size limit.')
    }
    $startPattern = '(?m)^' +
        [regex]::Escape($script:RemappingRegionStart) + '\r?$'
    $endPattern = '(?m)^' +
        [regex]::Escape($script:RemappingRegionEnd) + '\r?$'
    $startMatches = [regex]::Matches($Text, $startPattern)
    $endMatches = [regex]::Matches($Text, $endPattern)
    if ($startMatches.Count -ne 1 -or $endMatches.Count -ne 1 -or
        $endMatches[0].Index -lt
            ($startMatches[0].Index + $startMatches[0].Length)) {
        throw (Get-UpdateText '重映射代码区域标记无效。' `
            'The remapping source markers are invalid.')
    }
    $bodyStart = $startMatches[0].Index + $startMatches[0].Length
    $bodyLength = $endMatches[0].Index - $bodyStart
    return [pscustomobject]@{
        Prefix = $Text.Substring(0, $bodyStart)
        Body = $Text.Substring($bodyStart, $bodyLength)
        Suffix = $Text.Substring($endMatches[0].Index)
        Eol = $(if ($Text.Contains("`r`n")) { "`r`n" } else { "`n" })
    }
}

function Read-RemappingSourceText {
    param([Parameter(Mandatory = $true)][string]$Path)

    if (-not (Test-Path -LiteralPath $Path -PathType Leaf) -or
        (Get-Item -LiteralPath $Path).Length -gt 48MB) {
        throw (Get-UpdateText '可编辑映射源码不存在或超过大小上限。' `
            'The editable remapping source is missing or exceeds the size limit.')
    }
    return Get-Content -LiteralPath $Path -Raw -Encoding UTF8
}

function Merge-RemappingRegion {
    param(
        [Parameter(Mandatory = $true)][string]$SourcePath,
        [Parameter(Mandatory = $true)][string]$DestinationPath
    )

    $sourceParts = Get-RemappingRegionParts `
        (Read-RemappingSourceText $SourcePath)
    Set-RemappingRegionBody -Body $sourceParts.Body `
        -DestinationPath $DestinationPath
}

function Set-RemappingRegionBody {
    param(
        [Parameter(Mandatory = $true)][AllowEmptyString()][string]$Body,
        [Parameter(Mandatory = $true)][string]$DestinationPath
    )

    $destinationText = Read-RemappingSourceText $DestinationPath
    $destinationParts = Get-RemappingRegionParts $destinationText
    $normalizedBody = $Body.Replace("`r`n", "`n")
    $normalizedBody = $normalizedBody.Replace("`r", "`n")
    $normalizedBody = $normalizedBody.Replace("`n", $destinationParts.Eol)
    $updatedText = $destinationParts.Prefix + $normalizedBody +
        $destinationParts.Suffix
    # Validate the assembled text before publishing it, then replace in the
    # same directory so the final move remains on one volume.
    [void](Get-RemappingRegionParts $updatedText)
    $temporaryPath = $DestinationPath + '.mapping-merge-' +
        [Guid]::NewGuid().ToString('N') + '.tmp'
    try {
        [System.IO.File]::WriteAllText($temporaryPath, $updatedText,
            [System.Text.UTF8Encoding]::new($false))
        try {
            [System.IO.File]::Replace($temporaryPath, $DestinationPath,
                $null, $true)
        } catch {
...
function Invoke-Git {
    ...
    try {
        # Windows PowerShell converts native stderr lines into non-terminating
        # ErrorRecord objects. Under the script-wide Stop policy that would
        # abort a successful Git command before its exit code can be checked.
...
function Normalize-RemappingComparisonText {
    param([string]$Text)

    return $Text.Replace("`r`n", "`n").Replace("`r", "`n")
}

function Normalize-RemappingPrefixComparisonText {
    param([string]$Text)

    # The start-marker regex includes CR in a CRLF file but stops before LF;
    # that CR is a boundary delimiter, not part of the source prefix.
    if ($Text.EndsWith("`r")) {
        $Text = $Text.Substring(0, $Text.Length - 1)
    }
    return Normalize-RemappingComparisonText $Text
}

function Restore-GitSourceTransaction {
    param(
        [object]$Transaction,
    ...
    try {
        Invoke-Git @('-C', $Transaction.Root, 'reset', '--hard',
            $Transaction.PreviousCommit) | Out-Null
    } catch {
        $rollbackErrors.Add($_.Exception.Message)
    }
    try {
        Copy-Item -LiteralPath $Transaction.SourceBackupPath `
            -Destination $Transaction.EditableSourcePath -Force
    } catch {
        $rollbackErrors.Add($_.Exception.Message)
    }
    if ($rollbackErrors.Count -gt 0) {
        throw (Get-UpdateText `
            'Git 源码更新失败，且自动回滚未完全成功。入口备份保留在：{0}。原始错误：{1}；回滚错误：{2}' `
            'The Git source update failed and automatic rollback was incomplete. The entry backup remains at: {0}. Original error: {1}; rollback error: {2}' `
            @($Transaction.SourceBackupPath, $OriginalError.Exception.Message,
                ($rollbackErrors -join '；')))
    }
    if (Test-Path -LiteralPath $Transaction.SourceBackupPath) {
        Remove-Item -LiteralPath $Transaction.SourceBackupPath -Force
    }
}

function Complete-GitSourceTransaction {
    param([object]$Transaction)

    if ($Transaction -and
        (Test-Path -LiteralPath $Transaction.SourceBackupPath)) {
        Remove-Item -LiteralPath $Transaction.SourceBackupPath -Force `
            -ErrorAction SilentlyContinue
    }
}

function Restore-GitSourcePreparation {
    param([object]$Transaction)
...
function Prepare-GitSourceTransaction {
    param(
        [string]$Root,
    ...
    $sourceBackupPath = Join-Path $BackupDirectory `
        ('pre-git-update-entry-' + [Guid]::NewGuid().ToString('N') + '.ahk')
    Copy-Item -LiteralPath $EditablePath -Destination $sourceBackupPath
    ...
    try {
        # The GUI intentionally edits the tracked entry file. Restore the HEAD
        # copy temporarily, then verify that every difference outside the
        # managed mapping region was absent before allowing a fast-forward.
        Invoke-Git @('-C', $Root, 'checkout', '--', $EntryRelativePath) |
            Out-Null
        $originalParts = Get-RemappingRegionParts `
            (Read-RemappingSourceText $sourceBackupPath)
        $headParts = Get-RemappingRegionParts `
            (Read-RemappingSourceText $EditablePath)
        if ((Normalize-RemappingPrefixComparisonText $originalParts.Prefix) -cne
                (Normalize-RemappingPrefixComparisonText $headParts.Prefix) -or
            (Normalize-RemappingComparisonText $originalParts.Suffix) -cne
                (Normalize-RemappingComparisonText $headParts.Suffix)) {
            throw (Get-UpdateText `
                '入口源码在重映射区域之外含有本地修改，自动更新已取消。' `
                'The entry source has local changes outside the managed remapping region; automatic update was cancelled.')
        }
        ...
        $transaction.MappingRegionBody = $originalParts.Body
        return $transaction
    } catch {
        $preparationError = $_
        Restore-GitSourcePreparation $transaction
...
function Invoke-GitSourceUpdate {
    ...
    $transaction = Prepare-GitSourceTransaction -Root $root `
        -PreviousCommit $previousCommit -EntryRelativePath $entryRelativePath `
        -EditablePath $EditableSourcePath
    try {
        ...
        Set-RemappingRegionBody -Body $transaction.MappingRegionBody `
            -DestinationPath $EditableSourcePath
    } catch {
        $mergeError = $_
        Restore-GitSourceTransaction $transaction $mergeError
        throw $mergeError
    }
    return $transaction
}
...
function ConvertTo-CanonicalManagedRelativePath {
    ...
    if ([System.IO.Path]::IsPathRooted($canonical) -or
        $canonical.StartsWith('\') -or $canonical.EndsWith('\') -or
    ...
    foreach ($component in $canonical.Split('\')) {
        if (-not $component -or
    ...
    if ($canonical -in @('settings.ini', 'runtime.ini')) {
...
function Assert-ManagedRelativePath {
    ...
    [void](ConvertTo-CanonicalManagedRelativePath $RelativePath)
}

function Assert-NoReparsePointInManagedPath {
    param(
        [string]$Root,
    ...
    foreach ($component in $CanonicalRelativePath.Split('\')) {
        ...
        if (($item.Attributes -band [System.IO.FileAttributes]::ReparsePoint) `
            ...
            throw (Get-UpdateText '更新路径超出允许目录：{0}' `
                'Update path escapes the allowed directory: {0}' `
...
function Resolve-PathUnderRoot {
    ...
    $canonicalRelativePath = ConvertTo-CanonicalManagedRelativePath `
        $RelativePath
    $fullRoot = [System.IO.Path]::GetFullPath($Root).TrimEnd('\') + '\'
    $fullPath = [System.IO.Path]::GetFullPath(
        (Join-Path $Root $canonicalRelativePath))
    if (-not $fullPath.StartsWith($fullRoot,
            [System.StringComparison]::OrdinalIgnoreCase)) {
        throw (Get-UpdateText '更新路径超出允许目录：{0}' `
            'Update path escapes the allowed directory: {0}' @($RelativePath))
    }
    Assert-NoReparsePointInManagedPath $Root $canonicalRelativePath
    return $fullPath
}
...
function Assert-SafeUpdateArchive {
    param(
        ...
        [ValidateRange(1, 1073741824)]
        [long]$MaximumExpandedBytes = $script:MaximumExpandedArchiveBytes,
        [ValidateRange(1, 100000)]
        [int]$MaximumEntries = $script:MaximumArchiveEntries
    )
    ...
    try {
        if ($archive.Entries.Count -lt 1 -or
            ...
            throw (Get-UpdateText '更新包文件数量超过安全上限。' `
                'The update package file count exceeds the safety limit.')
        }
        ...
        foreach ($entry in $archive.Entries) {
            ...
            $directoryEntry = [string]::IsNullOrEmpty([string]$entry.Name)
            if ($directoryEntry) {
                $entryName = $entryName.TrimEnd('/', '\')
            }
            if (-not $entryName) {
                throw (Get-UpdateText '更新包包含不安全路径：{0}' `
                    'The update package contains an unsafe path: {0}' `
                    @([string]$entry.FullName))
            }
            $canonicalPath = ConvertTo-CanonicalManagedRelativePath $entryName
            if (-not $seenPaths.Add($canonicalPath)) {
                throw (Get-UpdateText '更新包包含重复路径：{0}' `
                    'The update package contains a duplicate path: {0}' `
                    @($canonicalPath))
            }
            [void](Resolve-PathUnderRoot $DestinationRoot $canonicalPath)
            if (-not $directoryEntry) {
                $expandedBytes += [long]$entry.Length
                if ($expandedBytes -gt $MaximumExpandedBytes) {
                    throw (Get-UpdateText '更新包解压后超过大小上限。' `
                        'The expanded update package exceeds the size limit.')
                }
            }
        }
    } finally {
...
function Assert-NoOverlappingPaths {
    ...
    $normalized = @($RelativePaths | ForEach-Object {
        ConvertTo-CanonicalManagedRelativePath ([string]$_)
    ...
    for ($index = 0; $index -lt $normalized.Count; $index++) {
        for ($otherIndex = $index + 1;
            ...
            if ($normalized[$otherIndex] -ieq $normalized[$index] -or
                $normalized[$otherIndex].StartsWith(
                    $normalized[$index] + '\',
                    [System.StringComparison]::OrdinalIgnoreCase)) {
...
function Install-ArchivePackage {
    ...
    $workRoot = Join-Path $env:TEMP `
        ("KeyMouseRemapperUpdate-" + [Guid]::NewGuid().ToString('N'))
    ...
    try {
        ...
        if (-not $expectedHash) {
            ...
            Invoke-ResilientDownload -Uri $ChecksumsUrl `
                -DestinationPath $checksumsPath -TimeoutSeconds 120 `
                -MaximumBytes 4MB | Out-Null
        ...
        Assert-SafeUpdateArchive -ArchivePath $packagePath `
        ...
        if ((Get-Item -LiteralPath $manifestPath).Length -gt
                $script:MaximumManifestBytes) {
            throw (Get-UpdateText '更新包清单超过大小上限。' `
                'The update package manifest exceeds the size limit.')
        }
        ...
        $manifestEntry = ConvertTo-CanonicalManagedRelativePath `
        ...
        if ($managedPaths.Count -eq 0 -or
            $managedPaths.Count -gt $script:MaximumArchiveEntries) {
        ...
        foreach ($relativePathValue in $managedPaths) {
            $relativePath = ConvertTo-CanonicalManagedRelativePath `
        ...
        if (-not $managedPathSet.Contains($manifestEntry)) {
        ...
        } elseif ([System.IO.Path]::GetExtension($manifestEntry) -ine '.ahk') {
            throw (Get-UpdateText '更新包清单与请求的版本或运行形态不一致。' `
                'Update package manifest does not match the requested release.')
        }
        ...
        foreach ($relativePath in $managedPathSet) {
            $destinationRelative = if ($relativePath -ieq
                $manifestEntry) { $entryDestination } else {
                $relativePath
            }
        ...
        if (Test-Path -LiteralPath $existingManifestPath -PathType Leaf) {
            if ((Get-Item -LiteralPath $existingManifestPath).Length -gt
                    $script:MaximumManifestBytes) {
            ...
            $existingManifestEntry = ConvertTo-CanonicalManagedRelativePath `
            ...
            foreach ($existingPathValue in @($existingManifest.managedPaths)) {
                $existingPath = ConvertTo-CanonicalManagedRelativePath `
                    ([string]$existingPathValue)
                ...
                $existingDestination = if ($existingPath -ieq
                    $existingManifestEntry) { $entryDestination } else {
                    $existingPath
                }
            ...
            if (-not $existingPathSet.Contains($existingManifestEntry)) {
        ...
        $backupRoot = Join-Path $root `
            ('.key-mouse-remapper-assistant-update-backup-' + [Guid]::NewGuid().ToString('N'))
        ...
        $pendingEntryPath = Join-Path $root `
            ('.key-mouse-remapper-assistant-entry-' + [Guid]::NewGuid().ToString('N') +
                '.tmp')
        try {
...
function Show-UpdateFailure {
    ...
    $logPath = Join-Path $env:TEMP 'KeyMouseRemapperUpdateErrors.log'
    ...
    try {
        ...
        $dialogTitle = Get-UpdateText '键鼠重映射小助手更新' `
            'Key Mouse Remapper Assistant Update'
...
function Assert-ApplyArguments {
    ...
    $editableSource = if ($EditableSourcePath) {
        [System.IO.Path]::GetFullPath($EditableSourcePath)
    } else { '' }
    if (-not (Test-Path -LiteralPath $root -PathType Container) -or
        -not (Test-Path -LiteralPath $entry -PathType Leaf) -or
        -not $editableSource -or
        -not (Test-Path -LiteralPath $editableSource -PathType Leaf)) {
    ...
    if ((Split-Path -Parent $editableSource).TrimEnd('\') -ne
            $root.TrimEnd('\') -or
        [System.IO.Path]::GetExtension($editableSource) -ine '.ahk') {
        throw (Get-UpdateText '可编辑映射源码不在安装目录的顶层。' `
            'The editable remapping source is not directly inside the installation directory.')
    }
    [void](Get-RemappingRegionParts (Read-RemappingSourceText $editableSource))
    ...
    Assert-TrustedReleaseUrl $BinaryUrl ($downloadBase +
        "key-mouse-remapper-assistant-$Version-windows-x64.zip") -Optional
    Assert-TrustedReleaseUrl $SourceUrl ($downloadBase +
        "key-mouse-remapper-assistant-$Version-source.zip") -Optional
    Assert-TrustedReleaseUrl $ChecksumsUrl ($downloadBase + 'SHA256SUMS.txt') `
        -Optional
...
function Remove-ApplyHelperDirectory {
    ...
    try {
        ...
        if ($scriptDirectory.StartsWith($tempPrefix,
            ...
            $directoryName -like 'KeyMouseRemapperUpdateApply-*') {
...
function Invoke-Apply {
    ...
    $gitTransaction = $null
    $mappingRegionBody = (Get-RemappingRegionParts `
        (Read-RemappingSourceText $EditableSourcePath)).Body
    ...
    try {
        if ($PackageKind -ne 'source-git') {
            Set-RemappingRegionBody -Body $mappingRegionBody `
                -DestinationPath $EditableSourcePath
        }
        ...
        try {
            if ($archiveTransaction) {
                ...
                Restore-GitSourceTransaction $gitTransaction $startupError
            }
        } catch {
    ...
    if ($archiveTransaction) {
        ...
        Complete-GitSourceTransaction $gitTransaction
    }
}
